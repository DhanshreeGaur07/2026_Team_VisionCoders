...
#NOTE: verifirewall-ctl only supports nano services with name of the format cp-nano-<service>
...
read_agent_run_status() # Initials - rars
{
    ...
    if [ "$1" = "start" ]; then
        if [ "$rars_output" = "running" ]; then
            echo "verifirewall Nano Agent watchdog started successfully"
        else
            echo "verifirewall Nano Agent is already running"
        fi
    else # "$1" = "stop"
        if [ "$rars_output" = "down" ]; then
            echo "verifirewall Nano Agent stopped successfully"
        else
            echo "verifirewall Nano Agent is not running"
        fi
    fi
}
...
{
    printf "Are you sure you want to uninstall verifirewall Nano Agent? (Y/N): " && read -r ua_confirm
    case $ua_confirm in
    [Yy] | [Yy][Ee][Ss]) ;;
    *) exit 1 ;;
    esac
    ...
    ${ua_uninstall_script}
    if test "$?" = "0"; then
        echo "verifirewall Nano Agent successfully uninstalled"
    else
        echo "Failed to uninstall verifirewall Nano Agent"
        exit 1
    fi
}
...
run_update_gradual_policy() # Initials - rugp
{
    ...
    rugp_gp_usage="Usage: verifirewall-ctl -gp|--set-gradual-policy [access-control|http-manager] <ip-ranges>"
    rugp_success_message="Gradual policy for $rugp_service_name was set successfully"
    if [ "$rugp_mod" = "delete" ]; then
        rugp_gp_usage="Usage: verifirewall-ctl -dg|--delete-gradual-policy [access-control|http-manager]"
...
run_set_traffic_recording_policy() # Initials - rstrp
{
    if [ "$1" != "off" ] && [ "$1" != "req_hdr" ] && [ "$1" != "req_body" ] && [ "$1" != "resp_hdr" ] && [ "$1" != "resp_body" ]; then
        printf "Error: Could not set up traffic recording.\nUsage: verifirewall-ctl <-tr|--traffic-recording> <off|req_hdr|req_body|resp_hdr|resp_body>\n"
        exit 1
    fi
...
{
    echo ""
    echo "For release notes and known limitations check: https://docs.verifirewall.io/release-notes"
    echo "For troubleshooting and support: https://verifirewall.io/support"
}
...
print_single_service_status() # Initials - psss
{
    ...
    echo "---- verifirewall $(format_nano_service_name "$psss_service_name") Nano Service ----"
...
run_load_settings() # Initials - rls
{
    ...
    if [ -z "$rls_service_to_update" ] || [ -z "$(get_nano_service_location_and_port "$rls_service_to_update")" ]; then
        ...
        printf "Usage: verifirewall-ctl <-lc|--load-config> <%b>\n" "$(get_installed_services '|')"
        exit 1
    fi
...
set_proxy() # Initials - sp
{
    ...
    if [ -z "$sp_proxy" ]; then
        ...
        ech "Usage: verifirewall-ctl <-sp|--set-proxy> <proxy>"
        exit 1
    fi
...
set_mode()
{
    ...
    if [ "$mode" != "offline_mode" ]; then
        if [ -n "$token" ] && [ -z "$fog_address" ]; then
            ...
            if [ "${var_token#"$us_prefix"}" != "${var_token}" ] || [ "${var_token#"$us_prefix_uppercase"}" != "${var_token}" ]; then
                ...
                echo "Failed to get fog address from toekn: ${var_token} - check if token is legal"
            fi
    ...
    if [ "$mode" = "online_mode" ]; then
        time_sleep=2
        time_out=60
        echo "Registering verifirewall Nano Agent to Fog.."
        until get_status_content | grep -q "Registration status: Succeeded"; do
            time_out=$(( time_out - time_sleep ))
            if [ $time_out -le 0 ]; then
                echo "verifirewall Nano Agent registration failed. Failed to register to Fog: $fog_address"
                exit 1
            fi
            sleep ${time_sleep}
        done
        echo "verifirewall Nano Agent is registered to $fog_address"
...
{
    if [ -z "$1" ]; then
        printf "Usage: verifirewall-ctl <-rs|--start-service> <%b>\n" "$(get_installed_services '|')"
        exit 255
    fi
...
{
    if [ -z "$1" ]; then
        printf "Usage: verifirewall-ctl <-qs|--stop-service> <%b>\n" "$(get_installed_services '|')"
        exit 255
    fi
