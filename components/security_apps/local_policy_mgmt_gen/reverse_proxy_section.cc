...
static const string nginx_configuration_path = "verifirewall-nginx-servers/";
...
{
    certs_path = getProfileAgentSettingWithDefault<string>("/etc/certs/", "verifirewall.reverseProxy.certs");
...
ReverseProxyBuilder::init()
{
    ...
    nginx_templates_path = getProfileAgentSettingWithDefault<string>(
        "/etc/nginx/nginx-templates/", "verifirewall.reverseProxy.nginxTemplates"
    );
