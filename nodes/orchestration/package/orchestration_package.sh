...
install_cp_nano_ctl()
{
    ...
    CP_NANO_CTL="cpnano"
    verifirewall_CTL="verifirewall-ctl"
    ...
    cp_exec "ln -s ${FILESYSTEM_PATH}/${SCRIPTS_PATH}/${verifirewall_CTL}.sh $USR_SBIN_PATH/${verifirewall_CTL}"
...
copy_orchestration_executable()
{
    ...
    cp_copy verifirewall-cloud-mgmt ${FILESYSTEM_PATH}/${SCRIPTS_PATH}/verifirewall-cloud-mgmt
    cp_copy verifirewall-cloud-mgmt-k8s ${FILESYSTEM_PATH}/${SCRIPTS_PATH}/verifirewall-cloud-mgmt-k8s
    cp_copy verifirewall-ctl.sh ${FILESYSTEM_PATH}/${SCRIPTS_PATH}/verifirewall-ctl.sh
...
install_orchestration()
{
    ...
    if [ -n "$var_upgrade_mode" ]; then
        upgrade_orchestration_policy
        cp_print "\nStarting upgrading of verifirewall Nano Agent [$INSTALLATION_TIME]" ${FORCE_STDOUT}
        install_cp_nano_ctl
        add_uninstall_script
    ...
    cp_print "\nStarting installation of verifirewall Nano Agent [$INSTALLATION_TIME]" ${FORCE_STDOUT}
    ...
    if [ -z "${var_token}" ] && [ ${var_hybrid_mode} = false ] && [ ${var_offline_mode} = false ] && [ -z ${EGG_MODE} ] && [ ${var_no_otp} = false ]; then
        ...
        while [ -z "$var_token" ]; do
            ...
            if [ "$attempts" -gt "$max_attempts" ]; then
                cp_print "Maximum attempts exceeded. verifirewall Nano Agent registration failed. Failed to get token" ${FORCE_STDOUT}
                exit 1
            fi
    ...
    cp_print "verifirewall Orchestration Nano Service installation completed successfully" ${FORCE_STDOUT}

    if [ $var_hybrid_mode = false ] && [ $var_offline_mode = false ] && [ $var_no_otp = false ] && [ $var_skip_registration = false ]; then
        time_sleep=2
        time_out=60
        cp_print "Registering verifirewall Nano Agent to Fog.." ${FORCE_STDOUT}
        until get_status_content | grep -q "Registration status: Succeeded"; do
            time_out=$(( time_out - time_sleep ))
            if [ $time_out -le 0 ]; then
                cp_print "verifirewall Nano Agent registration failed. Failed to register to Fog: $var_fog_address" ${FORCE_STDOUT}
                exit 1
            fi
            sleep ${time_sleep}
        done
        cp_print "verifirewall Nano Agent is registered to $var_fog_address" ${FORCE_STDOUT}
    fi
}
...
uninstall_orchestration()
{
    ...
    if [ ! -f "$uninstall_script" ]; then
        ...
        if [ ! -d "$cp_dir" ]; then
            echo "verifirewall Nano Agent is not installed"
            exit 1
        fi
    ...
    if test "$?" = "0"; then
        cp_print "verifirewall Nano Agent successfully uninstalled" ${FORCE_STDOUT}
    else
        cp_print "verifirewall Nano Agent failed to uninstall" ${FORCE_STDOUT}
        exit 1
    fi
}
