...
MS_Help()
{
  ...
  if test x"\$additional_args_help" != x; then
      install_usage="--install [additional arguments]    Install verifirewall Nano service"
  else
      install_usage="--install                           Install verifirewall Nano service"
  fi
...
With following options
  \$install_usage
  --uninstall                         Uninstall verifirewall Nano service
  --version                           verifirewall package version
...
\${helpheader}Makeself version $MS_VERSION
 ...
 2) Running \$0 :
  ...
  --install              Install verifirewall service
  --uninstall            Uninstall verifirewall service
  --pre_install_test     Preform pre installation test for verifirewall service
  --post_install_test    Preform post installation test for verifirewall service
  --version              verifirewall package version
...
EOLSM
	...
	;;
    --install)
	scriptargs="$SCRIPTARGS --install"
	shift
	break ;;
    --force)
	scriptargs="$SCRIPTARGS --force"
	shift
	;;
    --uninstall)
	scriptargs="$SCRIPTARGS --uninstall"
	shift
	;;
    --pre_install_test)
	scriptargs="$SCRIPTARGS --pre_install_test"
	shift
	;;
    --post_install_test)
	scriptargs="$SCRIPTARGS --post_install_test"
	shift
	;;
    --version)
...
if test x"$ARCH" = x"gaia" -a  -z "\`uname -a | grep -i x86\`"; then
	echo "Cannot install $ARCH(x86) installation package on a non compatible host" >&2
	exit 1

fi

if test x"$ARCH" = x"alpine" -a  -z "\`uname -a | grep -i x86\`"; then
	echo "Cannot install $ARCH(x86) installation package on a non compatible host" >&2
	exit 1

fi

if  test x"$ARCH" != x -a x"$ARCH" != x"gaia" -a x"$ARCH" != x"alpine" -a  -z "\`uname -a | grep -i \"$ARCH\"\`"  ; then
	echo "Cannot install $ARCH installation package on a non compatible host" >&2
	exit 1
fi
